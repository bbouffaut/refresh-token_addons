 ...
 function TokenProvider (url, options) {
  ...
  if(this.options.access_token){
    this.currentToken = {
      ...
      expiry_date: this.options.expiry_date
    };
  }
  ...
  if(this.currentToken && 'expires_in' in this.currentToken) {
    ...
    this.currentToken.expiry_date = new Date().getTime() + (this.currentToken.expires_in * 1000);
  }
}
 ...
 */
 TokenProvider.prototype.getToken = function (done) {
  if(this.currentToken && this.currentToken.expires_in_date > new Date()){
    return done(null, this.currentToken); //BBO: returns full currentToken object
  }
  ...
  request.post({
    ...
    //BBO: add expiry_date in the token
    this.currentToken.expiry_date = (new Date()).getTime() +  this.currentToken.expires_in * 1000;
